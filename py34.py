...
def future_code_warning(
    ...
    if item.lineno != item.end_lineno:
        log.warning(
            f"{item_description} ({version_str}+) found at lines {item.lineno} to {item.end_lineno}"
        )
    else:
        log.warning(f"{item_description} ({version_str}+) found at line {item.lineno}")


def clean_annotations_from_code(
    original_filename: str | PathLike[str],
    result_filename: str | PathLike[str] | None = None,
) -> str:
    """
    ...
    m: ast.Module = ast.parse(
        source=original_filename.read_text(), filename=str(original_filename)
    )
    ...
    def make_named_tuple_class(item: ast.ClassDef) -> ast.ClassDef:
        ...
        name: str = item.name
        members: list[ast.AnnAssign] = [
            statement for statement in item.body if isinstance(statement, ast.AnnAssign)
        ]
        ids: list[str] = [ann_assign.target.id for ann_assign in members]
        defaults: list[ast.expr] = [
            ann_assign.value for ann_assign in members if ann_assign.value is not None
        ]
        named_tuple_call: ast.Call = ast.Call(
            func=ast.Name(id=replace_typing_named_tuple_as[0]),
            args=[
        ...
        if any(v is not None for v in defaults):
            ...
            new_function: ast.FunctionDef = ast.FunctionDef(
                ...
                decorator_list=[],
                args=ast.arguments(
                    args=[ast.arg(arg="cls"), *[ast.arg(arg=_id) for _id in ids]],
                    defaults=defaults,
                    kw_defaults=[],
                    kwonlyargs=[],
                    posonlyargs=[],
    ...
    def fix_dataclass(item: ast.ClassDef) -> ast.ClassDef:
        ...
        for decorator in item.decorator_list:
            if (
                ...
                and decorator.id in import_dataclasses_dataclass_as
            ):
                continue
        ...
        if any(
            ...
            raise ValueError(
                f"Class {item.name} already has __init__ function; this cannot be right"
            )
        if any(
            isinstance(statement, ast.FunctionDef) and statement.name == "__setattr__"
            for statement in item.body
        ):
            raise ValueError(
                f"Class {item.name} already has __setattr__ function; this cannot be right"
            )

        _id: str
        members: list[ast.Assign] = [
            statement for statement in item.body if isinstance(statement, ast.Assign)
        ]
        assign: ast.Assign
        ann_members: list[ast.AnnAssign] = [
            statement for statement in item.body if isinstance(statement, ast.AnnAssign)
        ]
        ...
        for i, v in enumerate(defaults):
            if isinstance(v, ast.Attribute) and v.value.id in import_dataclasses_as:
                v = None
            if (
        ...
        init_function: ast.FunctionDef = ast.FunctionDef(
            ...
            decorator_list=[],
            args=ast.arguments(
                args=[ast.arg(arg="self"), *[ast.arg(arg=_id) for _id in ids]],
                defaults=defaults,
                kw_defaults=[],
                kwonlyargs=[],
                posonlyargs=[],
            ...
            body=[
                ast.Assign(
                    targets=[ast.Attribute(attr=_id, value=ast.Name(id="self"))],
    ...
    def joined_str_to_format_call(operator: ast.JoinedStr) -> ast.Call | ast.Constant:
        ...
        joined_str_to_format_call.arg_index = getattr(
            joined_str_to_format_call, "arg_index", 0
        )
                ...
                case ast.FormattedValue():
                    ...
                    match fs:
                        case None:
                            fs: None
                            format_string += (
                                f"{{__{joined_str_to_format_call.arg_index}}}"
                            )
                            joined_str_to_format_call.arg_index += 1
                        case ast.Constant():
                            fs: ast.Constant
                            format_string += const_fmt(
                                joined_str_to_format_call.arg_index,
                                fs.value,
                                level=len(calls),
                            )
                            joined_str_to_format_call.arg_index += 1
        ...
        for _args, _keywords in calls:
            call = ast.Call(
                func=ast.Attribute(value=call, attr="format"),
                args=_args,
                keywords=_keywords,
            )
        call = ast.Call(
            ...
            keywords=[
                ast.keyword(
                    arg=f"__{i + (joined_str_to_format_call.arg_index - len(args))}",
                    value=a,
                )
    ...
    def unpack_starred_elements(
        args: list[ast.expr],
    ) -> ast.expr | ast.BinOp | ast.Tuple | ast.Call:
        ...
        def make_left(
                ...
                if isinstance(cast(ast.Starred, elements[0]).value, ast.List):
                    return ast.Tuple(
                        elts=list(
                            check_expression(e)
                            for e in cast(
                                ast.List, cast(ast.Starred, elements[0]).value
                            ).elts
                        )
                    )
                else:
            ...
            if isinstance(elements[-1], ast.Starred):
                if isinstance(cast(ast.Starred, elements[-1]).value, ast.List):
                    return ast.BinOp(
                        left=make_left(elements[:-1]),
                        right=ast.Tuple(
                            elts=list(
                                check_expression(e)
                                for e in cast(
                                    ast.List, cast(ast.Starred, elements[-1]).value
                                ).elts
                            )
                        ),
                        op=ast.Add(),
    ...
    def unpack_starred_list(operator: ast.List) -> ast.expr:
        ...
        def make_left(
            elements: list[ast.expr],
        ) -> ast.expr | ast.BinOp | ast.List | ast.Call:
            if not any(isinstance(e, ast.Starred) for e in elements):
    ...
    def unpack_starred_set(operator: ast.Set) -> ast.expr:
        ...
        def make_left(
            elements: list[ast.expr],
        ) -> ast.expr | ast.BinOp | ast.Set | ast.Call:
            if not any(isinstance(e, ast.Starred) for e in elements):
    ...
    def unpack_starred_dict(operator: ast.Dict) -> ast.expr | ast.Call:
        def get_items(d: ast.Dict | ast.Name) -> ast.Call:
            return ast.Call(
                func=ast.Attribute(attr="items", value=d), args=[], keywords=[]
            )

        def make_tuple_from_dict(d: ast.Dict | ast.Name) -> ast.Call:
            return ast.Call(func=ast.Name(id="tuple"), args=[get_items(d)], keywords=[])

        def make_tuple_from_kv(
            keys: list[ast.expr | None], values: list[ast.expr | None]
        ) -> ast.Call:
            return make_tuple_from_dict(
                ast.Dict(
                    keys=list(check_expression(key) for key in keys),
        ...
        def make_left(
            ...
            if len(keys) == 1 and keys[-1] is None:
                match values[-1]:
                    case ast.Dict():
                        return make_tuple_from_kv(
                            cast(ast.Dict, values[-1]).keys,
            ...
            if keys[-1] is None:
                return ast.BinOp(
                    left=make_left(keys[:-1], values[:-1]),
                    right=make_tuple_from_kv(
                        cast(ast.Dict, values[-1]).keys,
                        cast(ast.Dict, values[-1]).values,
                    ),
                    op=ast.Add(),
            ...
            for i in range(len(keys) - 1, -1, -1):
                if keys[i] is None:
                    break
            return ast.BinOp(
                left=make_left(keys[: (i + 1)], values[: (i + 1)]),
    ...
    def check_expression(operator: ast.expr) -> ast.expr:
        ...
        for field in getattr(operator, "_fields", ()):
            ...
            if isinstance(operator_field, ast.expr):
                ...
                if all(
                    isinstance(field_item, ast.expr | None)
                    ...
                    isinstance(field_item, ast.keyword) for field_item in operator_field
                ):
                    pass  # see ast.Call below
                elif all(
                    isinstance(field_item, ast.cmpop) for field_item in operator_field
                ):
                    pass  # no changes since Python 3.4
                elif all(
                    isinstance(field_item, ast.comprehension)
                    for field_item in operator_field
                ):
                    pass  # no changes since Python 3.4
                else:  # non-homogeneous list
                    log.error(
                        f"do not know what to do with list {operator_field} of operator {operator}"
                    )
            else:
        ...
        match operator:
            ...
            case ast.Call():
                operator: ast.Call
                if (
                    isinstance(operator.func, ast.Name)
                    and operator.func.id in import_typing_cast_as
                ):
                    operator: ast.expr = check_expression(operator.args[1])
                elif (
                    isinstance(operator.func, ast.Attribute)
                    ...
                    if any(
                        isinstance(arg, ast.Starred) for arg in operator.args[:-1]
                    ):  # except the last arg
                        future_code_warning(
                            "unnamed arguments after *expression", (3, 5), operator
                        )
                ...
                if isinstance(operator.value, ast.Name):
                    if operator.value.id in import_math_as:
                        if operator.attr in ("inf", "nan"):
                            future_code_warning(
                                f"`{operator.value.id}.{operator.attr}` expression",
                                (3, 5),
                                operator,
                            )
                            operator: ast.Call = ast.Call(
                                func=ast.Name(id="float"),
                        ...
                        if operator.attr == "TYPE_CHECKING":
                            future_code_warning(
                                f"`{operator.value.id}.{operator.attr}` expression",
                                TYPING_CLASSES_VERSIONS.get(operator.attr, (3, 5)),
                                operator,
                            )
            ...
            case ast.Name():
                ...
                if operator.id in import_math_nan_as:
                    future_code_warning("`math.nan` expression", (3, 5), operator)
                    operator: ast.Call = ast.Call(
                        func=ast.Name(id="float"),
                    ...
                    future_code_warning("`math.inf` expression", (3, 5), operator)
                    operator: ast.Call = ast.Call(
                        func=ast.Name(id="float"),
                    ...
                    future_code_warning(
                        "`ModuleNotFoundError` exception", (3, 6), operator
                    )
            ...
            case ast.Tuple():
                ...
                if any(isinstance(e, ast.Starred) for e in operator.elts):
                    future_code_warning(
                        "starred expression as not an assignment target",
                        (3, 5),
                        operator,
                    )
    ...
    def clean_body(
        body: list[ast.stmt] | list[ast.excepthandler],
    ) -> list[ast.stmt] | list[ast.excepthandler]:
        if not body:
            return body
        ...
        for item in body:
            match item:
                case ast.ImportFrom():
                    ...
                    match item.module:
                        case "__future__":
                            no_annotations_names: list[ast.alias] = [
                                name
                        ...
                        case "typing":
                            ...
                            import_typing_overload_as.extend(
                                [
                                    name.asname or name.name
                                    for name in item.names
                            ...
                            import_typing_generic_as.extend(
                                [
                                    name.asname or name.name
                                    for name in item.names
                            ...
                            if any(name.name == "TYPE_CHECKING" for name in item.names):
                                cleaned_body.append(
                                    cast(
                                        ast.stmt,
                                        ast.Assign(
                                            targets=[ast.Name(id="TYPE_CHECKING")],
                            ...
                            for name in item.names:
                                if name.name in TYPING_CLASSES_VERSIONS:
                                    future_code_warning(
                                        f"`typing.{name.name}` import",
                                        TYPING_CLASSES_VERSIONS[name.name],
                                        item,
                                    )
                                    from_typing_import.append(name.asname or name.name)
                                else:
                                    if hasattr(collections, name.name) or hasattr(
                                        collections.abc, name.name
                                    ):
                                        collections_replacements.append(name)
                                    elif hasattr(
                                        collections, name.name.casefold()
                                    ) or hasattr(collections.abc, name.name.casefold()):
                                        name.name = name.name.casefold()
                                        collections_replacements.append(name)
                                    else:
                                        future_code_warning(
                                            f"`typing.{name.name}` import", (3, 5), item
                                        )
                                        from_typing_import.append(
                                            name.asname or name.name
                                        )
                            if collections_replacements:
                                item.module = (
                                    "collections"  # no `collections.abc` in Python 3.4
                                )
                        ...
                        case "dataclasses":
                            if any(name.name == "dataclass" for name in item.names):
                                no_dataclass_names: list[ast.alias] = [
                                    name
                                    for name in item.names
                                    if name.name != "dataclass"
                                ]
                                import_dataclasses_dataclass_as.extend(
                                    [
                                        name.asname or name.name
                                        for name in item.names
                                        if name.name == "dataclass"
                        ...
                        case "os":
                            if any(name.name == "PathLike" for name in item.names):
                                no_path_like_names: list[ast.alias] = [
                                    name
                                    for name in item.names
                                    if name.name != "PathLike"
                                ]
                                import_os_path_like_as.extend(
                                    [
                                        name.asname or name.name
                                        for name in item.names
                                        if name.name == "PathLike"
                        ...
                        case "math":
                            ...
                            if any(name.name == "nan" for name in item.names):
                                no_nan_names = [
                                    name for name in item.names if name.name != "nan"
                                ]
                                import_math_nan_as.extend(
                                    [
                                        name.asname or name.name
                                        for name in item.names
                                        if name.name == "nan"
                            ...
                            if any(name.name == "inf" for name in item.names):
                                no_nan_inf_names = [
                                    name for name in no_nan_names if name.name != "inf"
                                ]
                                import_math_inf_as.extend(
                                    [
                                        name.asname or name.name
                                        for name in item.names
                                        if name.name == "inf"
                ...
                case ast.Import():
                    ...
                    if any(
                        name.name == "typing" or name.name.startswith("typing.")
                        for name in item.names
                    ):
                        new_names = [
                            name
                            for name in item.names
                            if name.name != "typing"
                            and not name.name.startswith("typing.")
                        ]
                        future_code_warning("`typing` module", (3, 5), item)
                        import_typing_as.extend(
                            [
                                name.asname or name.name
                                for name in item.names
                                if name.name == "typing"
                        ...
                        if any(name.name == "typing.NamedTuple" for name in item.names):
                            replace_typing_named_tuple_as.append(
                    ...
                    if any(name.name.startswith("typing.") for name in item.names):
                        ...
                        for name in item.names:
                            ...
                            if name.name in TYPING_CLASSES_VERSIONS:
                                ...
                                if hasattr(
                                    collections.abc, name.name.removeprefix("typing.")
                                ):
                                    name.name = "collections." + name.name.removeprefix(
                                        "typing."
                                    )
                                elif hasattr(
                                    collections.abc,
                                    name.name.removeprefix("typing.").casefold(),
                                ):
                                    name.name = (
                                        "collections."
                                        + name.name.removeprefix("typing.").casefold()
                    ...
                    if any(
                        name.name == "dataclasses"
                        or name.name.startswith("dataclasses.")
                        for name in item.names
                    ):
                        new_names = [
                            name
                            ...
                            and not name.name.startswith("dataclasses.")
                        ]
                        future_code_warning("`dataclasses` module", (3, 7), item)
                        import_dataclasses_as.extend(
                            [
                                name.asname or name.name
                                for name in item.names
                                if name.name == "dataclasses"
                    ...
                    if any(name.name == "os.PathLike" for name in item.names):
                        ...
                        new_names = [
                            name for name in item.names if name.name != "os.PathLike"
                        ]
                    ...
                    if any(name.name == "math.inf" for name in item.names):
                        ...
                        new_names = [
                            name for name in item.names if name.name != "math.inf"
                        ]
                        future_code_warning("`math.inf` import", (3, 5), item)
                        if new_names:
                            item.names = new_names
                    ...
                    if any(name.name == "math.nan" for name in item.names):
                        import_math_as.append("math")
                        new_names = [
                            name for name in item.names if name.name != "math.nan"
                        ]
                        future_code_warning("`math.nan` import", (3, 5), item)
                        if new_names:
                            item.names = new_names
                ...
                case ast.AnnAssign():
                    ...
                    if item.value is None:
                        continue
                    item: ast.Assign = ast.Assign(
                        targets=[item.target], value=item.value, lineno=item.lineno
                    )
                case ast.Assign():
                    """
                    ...
                    if isinstance(item.value, ast.Call):
                        ...
                        if isinstance(item.value.func, ast.Attribute):
                            if hasattr(typing, item.value.func.attr) and isinstance(
                                item.value.func.value, ast.Name
                            ):
                ...
                case ast.AsyncFunctionDef():
                    ...
                    if any(
                        ...
                        (
                            isinstance(decorator, ast.Name)
                            and decorator.id in import_typing_overload_as
                        )
                        for decorator in item.decorator_list
                    ):
                    ...
                    item.args.defaults = [
                        check_expression(default) for default in item.args.defaults
                    ]
                    item.args.kw_defaults = [
                        check_expression(default) for default in item.args.kw_defaults
                    ]
                    item.returns = None
                ...
                case ast.AsyncWith() | ast.With():
                    ...
                    for with_item in item.items:
                        with_item.context_expr = check_expression(
                            with_item.context_expr
                        )
                        if with_item.optional_vars is not None:
                            with_item.optional_vars = check_expression(
                ...
                case ast.ClassDef():
                    ...
                    if any(
                        ...
                        and base.id in import_typing_named_tuple_as
                        for base in item.bases
                    ):
                        item = make_named_tuple_class(item)
                    if any(isinstance(base, ast.Subscript) for base in item.bases):
                        item = make_base_class_generic(item)
                    if any(
                        isinstance(base, ast.Name)
                        and base.id in import_typing_generic_as
                        for base in item.bases
                    ):
                        item.bases = [
                            base
                            ...
                            and base.id not in import_typing_generic_as
                        ]
                    if any(
                        (
                            isinstance(decorator, ast.Attribute)
            ...
            for field in getattr(item, "_fields", ()):
                ...
                if isinstance(item_field, ast.expr):
                    ...
                    if all(
                        isinstance(field_item, ast.stmt) for field_item in item_field
                    ):
                        setattr(item, field, clean_body(item_field))
                    elif all(
                        isinstance(field_item, ast.excepthandler)
                        for field_item in item_field
                    ):
                        setattr(item, field, clean_body(item_field))
                    elif all(
                        isinstance(field_item, ast.expr) for field_item in item_field
                    ):
                        setattr(item, field, list(map(check_expression, item_field)))
                    elif all(
                        isinstance(field_item, ast.keyword) for field_item in item_field
                    ):
                        setattr(item, field, list(map(check_expression, item_field)))
                    elif all(
...
if __name__ == "__main__":
    ...
    def _main() -> None:
        ...
        parser.add_argument(
            "-p", "--print", action="store_true", help="copy the result into stdout"
        )
        parser.add_argument(
            "-o",
            "--out",
        ...
        for filename in cl_args.file:
            ...
            if cl_args.print:
                print(
                    clean_annotations_from_code(
                        filename, filename.parent / cl_args.out / filename.name
                    )
                )
            else:
                clean_annotations_from_code(
