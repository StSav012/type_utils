# coding: utf-8
import re
from typing import Any, Dict, Final, Iterable, List, Set, Tuple
...
def name_type(an_object: Any) -> str:
    ...
    def _union(types: Iterable[str]) -> str:
        ...
        types = (
            sorted(
                t for t in types if t != none_type_str and t.isalnum()
            )  # first, simple types except None
            + sorted(t for t in types if not t.isalnum())  # then, complex types
            + ([none_type_str] * (none_type_str in types))
        )  # finally, None, if any
